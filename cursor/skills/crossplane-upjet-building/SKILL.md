...
- **Publish to Upbound:** On tag push `v*`: install Up CLI (`curl -sL "https://cli.upbound.io" | sh`), `up login -t "${{ secrets.UPBOUND_TOKEN }}"`, then `up project build --repository "<image_path>"` and `up project push --repository "<image_path>" --tag="${{ github.ref_name }}"`. Require `UPBOUND_TOKEN` secret; fail fast with a clear message if missing.
