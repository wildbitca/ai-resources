...
**Shared resources:** **`ai-resources`** repository (`cursor/` subtree for rules, skills, workflows, AGENTS.md). Run **`agent-kit self-update`** from the repo — see **`README.md`** at repo root.
...
|-------------|---------------|
| Feature (any domain) | `$AGENT_KIT/cursor/workflows/_feature-template.workflow.yaml` |
| Bugfix (any domain) | `$AGENT_KIT/cursor/workflows/_bugfix-template.workflow.yaml` |
| Security audit / pen test | `$AGENT_KIT/cursor/workflows/security-devsecops.workflow.yaml` |
| Explore / plan (any domain) | `$AGENT_KIT/cursor/workflows/explore-and-plan.workflow.yaml` |
{{EXTRA_WORKFLOWS}}
...
Ephemeral research, plans, and validation reports live under **`specs/knowledge/`** (same repo tree as living specs). **`specs/features/`, `specs/cross-cutting/`, and `specs/architecture/` are the product source of truth**; promote content from knowledge into those paths, then remove obsolete knowledge files (see `specs/knowledge/README.md` if present).
