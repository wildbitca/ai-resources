...
**Apply this skill when:** The user asks to maintain tf-modules, release modules, upgrade providers, sync dependent projects, normalize changelogs, or bring the Terraform estate up to a “professional set” of standards. **No parameters = do everything:** run all upgrades, inits, full release, and refresh across all repos and dependents; only limit scope (dry-run, projects only, no release) when the user explicitly asks.

---

## Scope — all known Terraform code

**The maintainer is responsible for maintaining all known Terraform code**, not just the tf-modules. This includes:

- **tf-modules** (all `tf-module-*` repos under the parent, e.g. tf-modules-parent/tf-modules): upgrade providers and internal refs, release (commit, CHANGELOG, tag, push), changelog, refresh.
- **Dependent projects** (e.g. my-infra-gitops, my-project/ops, and any other project consuming private modules): upgrade **providers** and **internal module versions** (`app.terraform.io/<your-org>/.../module`) to the latest tags, run `terraform init -upgrade` and `validate`, and **commit and push** the changes generated by the upgrade in each dependent.

**Upgrading dependents:** When running upgrade on a dependent project, use `upgrade-providers.py` with the **third argument (modules_root)** = path to the tf-modules directory, so that **private module versions** are updated to the latest published tag — not just providers. This ensures plans (CI, Terraform Cloud, remote) use the current module behavior.

**Changes in dependents:** If the upgrade leaves uncommitted changes in a dependent (e.g. `version = "~> 3.1"` → `"~> 4.0"` in main.tf), the maintainer must **commit and push** those changes in that repo. **version-commit.py** (with `--parent-dir`) does this **automatically** at the end of the run: after the release and refresh, it commits and pushes in each dependent with changes, so the **next TFC/CI run** already uses the module versions with the fix (e.g. module-base v4.0.5).

---

## Golden line — canonical workflow
...
6. **Dependents: commit and push:** If any dependent project has uncommitted changes (e.g. module versions updated by upgrade-providers), commit and push in that repo so all known Terraform code stays maintained.

**Rule:** No parameter = do **all** upgrades, all inits, full release, refresh, and commit+push in dependents when the upgrade changed their files. Do not omit steps unless the user explicitly asks to limit (e.g. "dry-run only", "projects only", "no release").
...
**Typical layout:** tf-modules live under e.g. `tf-modules-parent/tf-modules/` (each `tf-module-*` is a git repo). Dependent projects live alongside (e.g. `tf-modules-parent/infra-gitops`, `my-project/ops`). To maintain **all** known Terraform code, pass all dependents in `--dependent-projects` (comma-separated paths), or set `DEPENDENT_PROJECTS` (e.g. `/path/to/infra-gitops,/path/to/my-project/ops`) before running `maintain.sh`.
