...
- **Node**: `npm audit --audit-level=high` + `trivy fs package-lock.json`
- **Dart/Flutter**: `dart pub outdated --json` + `trivy fs pubspec.lock`
- **Go**: `go list -m -u all` + `trivy fs go.sum`
- **Java**: OWASP Dependency-Check + `trivy fs .`
- **Python**: `pip-audit` + `trivy fs requirements.txt`
- **Rust**: `cargo audit` + `trivy fs Cargo.lock`
- **All**: `trivy fs . --scanners vuln --severity HIGH,CRITICAL --exit-code 1`
...
### 7. Secret Detection (Automated)

Run Gitleaks on every commit/PR:

```bash
gitleaks detect --source . --verbose --redact
```

Pre-commit hook: add `gitleaks` to `.pre-commit-config.yaml` or `lefthook.yml`.

### 8. SAST Scan (Multi-Language)

```bash
semgrep scan --config auto --config p/owasp-top-ten --config p/security-audit . --error
```

For Dart/Flutter where Semgrep coverage is limited, supplement with:
- **DCM** security-related rules (`dcm analyze lib`)
- **SonarQube** with Dart plugin (SAST for Dart 3.x)
- **Flutter Security Analyser** VS Code extension (46+ OWASP MASVS rules)

### 9. Container Security

```bash
hadolint Dockerfile                                    # Lint Dockerfile
trivy image <image:tag> --severity HIGH,CRITICAL       # CVE scan built image
dockle <image:tag>                                     # CIS benchmark
```
