...
**Config:** Role definitions may live in a shared AI resources repo or `~/.cursor/`. Handoff and plan live in the **current workspace** (e.g. `<repo>/.agent-output/handoff*.md`, `<repo>/PLAN.md`).
...
- Before starting: check `knowledge/decisions/` for architectural decisions that may constrain the implementation
- After review: if significant patterns or anti-patterns are identified, note them in `knowledge/searchable/` for future reference
