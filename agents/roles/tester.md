...
**Config:** Role definitions may live in a shared AI resources repo or `~/.cursor/`. Handoff lives in the **current workspace** (e.g. `<repo>/handoff.md`).
...
- Before starting: check `knowledge/research/` for known edge cases or test strategies for this area
- After test execution: save the test report to `knowledge/searchable/` for future reference and traceability
